                        ...
                        <div class="col-md-6">
                                ...
                                <select class="form-control" name="product_category_id" id="">
                                    ...
                                    @foreach ($categories as $category)
                                        @if ($category->name == 'Men')
                                            <option value="1"
                                                {{ $product->product_category_id == '1' ? 'selected' : '' }}>
                                                {{ $category->name }}</option>
                                        @elseif ($category->name == 'Women')
                                            <option value="2"
                                                {{ $product->product_category_id == '2' ? 'selected' : '' }}>
                                                {{ $category->name }}</option>
                                        @elseif ($category->name == 'Kid')
                                            <option value="3"
                                                {{ $product->product_category_id == '3' ? 'selected' : '' }}>
                                                {{ $category->name }}</option>
                                        @else
                                ...
                                <select class="form-control" name="brand_id" id="">
                                    ...
                                    @foreach ($brands as $brand)
                                        @if ($brand->name == 'Calvin Klein')
                                            <option value="1" {{ $product->brand_id == '1' ? 'selected' : '' }}>
                                                {{ $brand->name }}</option>
                                        @elseif ($brand->name == 'Diesel')
                                            <option value="2" {{ $product->brand_id == '2' ? 'selected' : '' }}>
                                                {{ $brand->name }}</option>
                                        @elseif ($brand->name == 'Polo')
                                            <option value="3" {{ $product->brand_id == '3' ? 'selected' : '' }}>
                                                {{ $brand->name }}</option>
                                        @elseif ($brand->name == 'Tommy Hilfiger')
                                            <option value="4" {{ $product->brand_id == '4' ? 'selected' : '' }}>
                                                {{ $brand->name }}</option>
                                        @else
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="tag" id="">
                                    <option selected value="">Ấn vào để chọn</option>
                                    <option selected value="Clothing" {{ $product->tag == 'Clothing' ? 'selected' : '' }}>
                                        Clothing</option>
                                    <option value="HandBag" {{ $product->tag == 'HandBag' ? 'selected' : '' }}>HandBag
                                    </option>
                                    <option value="Accessorie" {{ $product->tag == 'Accessorie' ? 'selected' : '' }}>
                                        Accessorie</option>
                                </select>
                                @error('tag')
                                    <span class="text-danger">{{ $message }}</span>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="featured" id="">
                                    <option value="1" {{ $product->featured == '1' ? 'selected' : '' }}>
                                        Thường</option>
                                    <option value="0" {{ $product->featured == '0' ? 'selected' : '' }}>Nổi bật
                                    </option>
                                </select>
                            </div>
                        </div>
